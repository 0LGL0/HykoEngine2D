...
namespace HKCR {
	App::App() {
		...
		entity6.getComponent<HK::TransformComponent>() = HK::TransformComponent(glm::vec2(0.0f, 0.0f), glm::vec2(70.0f, 50.0f), 0.0f);
		...
		entity.getComponent<HK::TransformComponent>() = HK::TransformComponent(glm::vec2(140.0f, 20.0f), glm::vec2(30.0f, 30.0f), 0.0f);
		...
		entity3.getComponent<HK::TransformComponent>() = HK::TransformComponent(glm::vec2(-570.0f, -20.0f), glm::vec2(30.0f, 30.0f), 0.0f);
		...
		while (!glfwWindowShouldClose(nWindow)) {
			...
			if (m_gameWindow->getEvents()->WindowResizing) {
				const auto w = m_gameWindow->getWidth();
				const auto h = m_gameWindow->getHeight();
				...
				//m_sceneManager->getCurrentScene()->getCurrentCamera()->getComponent<HK::CameraComponent>().updateProjectionMat(w, h);
			}
	...
	App::~App() {
		...
		// Deleted in the scene destructor
